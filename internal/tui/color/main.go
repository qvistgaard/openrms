...
func CustomStyles() table.Styles {
...
// RedStyles returns a set of special style definitions for this table.
func RedStyles() table.Styles {
	s := table.DefaultStyles()
	s.Header = s.Header.
		BorderStyle(lipgloss.NormalBorder()).
		BorderForeground(lipgloss.Color("240")).
...
func (m model) View() string {
	// get selectedrow, return a Row which is a []string then at second elem if it's Cairo, then make it red
	if len(m.table.SelectedRow()) > 2 && m.table.SelectedRow()[1] == "Cairo" {
		m.table.SetStyles(RedStyles())
	}
